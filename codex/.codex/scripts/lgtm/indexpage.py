...
MAX_RECENTS = 5
MAX_WORKTREES = 8
# Convention owned by Codex harness worktree scaffolding: if this marker
# renames, the filter goes inert (worktrees reappear as clutter, no error).
AGENT_SCRATCH_MARKERS = ("/.codex/worktrees/agent-", "/.claude/worktrees/agent-")
...
def _filter_worktrees(pairs: list[tuple[str, str]]) -> tuple[list[tuple[str, str]], int]:
    ...
    real = [(p, b) for p, b in pairs if not any(marker in p for marker in AGENT_SCRATCH_MARKERS)]
