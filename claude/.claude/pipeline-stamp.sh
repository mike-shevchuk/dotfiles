...
#
# State lives at /tmp/claude/pipeline-<sanitized-branch>.json (matches the
# statusline's /tmp/claude cache convention). Keyed by git BRANCH, so every
...
case "$cmd" in
  ...
  refresh-pr)
    ...
    # JSON; the PR badge + 🐛 count live in /tmp/claude/pr-cache-<cwd>.txt.
    ...
    prefix="/tmp/claude/pr-cache-${canon//\//-}"
    printf '%s\n' "$line" > "${prefix}.txt"
    count=1
