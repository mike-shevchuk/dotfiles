...
The comment→Claude live loop bus:

    browser ──POST /comment──▶ inbox.jsonl   (Claude watcher picks it up)
...
(Mike's questions from inbox, Claude's answers from outbox). The event id is
