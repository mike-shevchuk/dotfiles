...
~/.claude/pipeline-stamp.sh show          # state file (may be empty — then treat all as pending)
...
```bash
~/.claude/pipeline-stamp.sh stamp <step> <src>   # src=commit:<sha> when the evidence is a commit
```
