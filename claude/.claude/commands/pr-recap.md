...
Render to **terminal stdout** (no posting anywhere). Markdown rendered by the Claude Code TUI gives you syntax highlighting on code fences with a language tag, so always tag fences (`python`, `bash`, `diff`, `json`, `yaml`).
