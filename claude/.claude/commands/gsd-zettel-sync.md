...
Helper: `~/.claude/gsd-zettel-sync.sh` (sync|dry|context|path|help).
...
2. Show the preview, then on Mike's go (or if `$ARGUMENTS` already says `sync`):
   ```bash
   ~/.claude/gsd-zettel-sync.sh sync
   ```
...
```bash
~/.claude/gsd-zettel-sync.sh context <keyword> [<keyword> ...]
```
